...
- Реліз: `2026-09-21.09b90a60fcee`
- Продукт: `starter` — Novsky Starter
- Ревізія вихідного комплекту: `09b90a60fcee66f670462f1a68a60b7b092c7441`
...
продуктів; точний склад цього пакета наведено нижче.
    ...
    CI: перевірки запускаються лише вручну, гейт іде локально
    Тест: модульний блок корпоративних сесій шукається за новою умовою
    Захист від вкладеного Claude: ловить запуск і через bash -c
    Встановлення: перевірка user namespaces до будь-яких змін і прапорець MODULE_TELEGRAM_CORPORATE
    Налаштування: керований шлях settings-model-guard у відсортованій позиції політики
    Корпоративний режим: пауза відмовляє одразу, якщо resume після неї був би заблокований
    Налаштування: нову модель записує лише після того, як CLI її прийняв
