...
- Реліз: `2026-09-20.4e4592616fd3`
- Продукт: `starter` — Novsky Starter
- Ревізія вихідного комплекту: `4e4592616fd347e32415c729dd4a7181effab1f7`
- Дата релізу: `2026-09-20`
...
продуктів; точний склад цього пакета наведено нижче.
    ...
    test: keep rescue timers from racing worker cleanup
    test: чекати звільнення слотів після тайм-ауту контексту
    test: чекати підтвердження повідомлення про ліміт
    test: завершувати планувальники перед видаленням тимчасових баз
    test: завершувати конкурентний старт на Python 3.12 і перевіряти всі CI-шарди
    ci: дати повній перевірці відновлення завершитися
    test(telegram): уточнення завершує звичайний хід
    fix(telegram): продовжувати звичайний діалог без обов'язкового Reply
    fix(telegram): не поновлювати минулі ліміти після перезапуску
    fix(telegram): bind restored callbacks and coordinate Stop completion
    fix(runtime): preserve Telegram requests and context across interruptions
    fix: фонові відповіді завершують свій початковий запит
    Документи: імʼя агента — Кеш, без відмінювання
    Тести: ширше вікно квитанції в тесті лестниці бездіяльності — секунди замало на завантаженій машині
    Документи: журнал розкатки — смоук після переходу на тіньовий режим
    Документи: журнал розкатки квитанцій — Кеш і Кнопа в тіньовому режимі
    Telegram: захват повідомлення записує сесію зі штампом, якщо SessionStart випередив приймач
    Telegram: перемикач старшинства доставки з тіньовим режимом і звітом розбіжностей
    Telegram: завислий хід перериває приймач, а не перезапуск служби; власник має /stop
    Документи: пред-польотні факти для розкатки квитанцій на Кеші та Кнопі
    Telegram: хід без відповіді повертається моделі один раз, потім людина отримує короткий підсумок
    Тести: рестарт служби у сценаріях сповіщень несе новий штамп лаунчера
    Тести: злиття гілок квитанцій і сповіщень зберігає обидва набори перевірок надійності
    Telegram: доставку відповіді засвідчує сам приймач; мовчання в групі — інструмент no_reply
    Telegram: перерваний запит отримує сповіщення, «печатає» триває до кінця ходу, реакція «отримано» за замовчуванням
    Збірка: інструмент оновлення хешів приймача не потрапляє до клієнтського комплекту
    Telegram: приймач веде журнал ходів, хуки лише позначають початок і кінець
    Дослідження: проба контракту хуків на живому боксі та журнал розкатки квитанцій
    Оновлення: зняті комплектом крон-рядки прибираються з живих установок
    Тести: приймач Telegram перевіряється запуском зі стабом Bot API
    Інструменти: одна команда оновлює закріплені хеші приймача
    Документи: план квитанцій доставки та політики підтверджень
    telegram: геомітка чи контакт доходять до агента; «друкує» гасне після тихого закриття
    черга: службові нотатки моделі та NO_REPLY більше не досилаються людині як відповідь
    черга: агент не зависає після мовчання в групі й не плутає опис інструмента з кінцем підписки
