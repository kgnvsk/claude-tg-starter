...
# node 22+ для npx / skills add / playwright-MCP — гарантуємо ТУТ (Фазу 1 могли пропустити):
node -e 'process.exit(+process.versions.node.split(".")[0]>=22?0:1)' 2>/dev/null || \
  { echo "  node<22 → ставлю 22.x з nodesource"; curl -fsSL https://deb.nodesource.com/setup_22.x | bash - && apt-get install -y nodejs; } || \
  echo "  WARN: node 22 не встановився (мережа?) — постав вручну"

echo "[3c/6] Google Chrome (браузер для playwright-MCP: скриншоти/навігація бота)"
if ! command -v google-chrome >/dev/null 2>&1 && ! command -v google-chrome-stable >/dev/null 2>&1; then
  wget -q -O /tmp/gc.deb https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y -q /tmp/gc.deb >/dev/null; rm -f /tmp/gc.deb
fi
google-chrome --version 2>/dev/null || echo "  WARN: Chrome не встановився — браузер-скіли не працюватимуть"
