...
**АНТИ-ПАТТЕРН (убивает креатив в «пейнт») — так НЕ надо:** микро-режиссура геометрии — «TOP 38%… THREE solid color bars edge-to-edge… FLAT solid yellow on FLAT solid black… sharp corners… exact pixel layout». Плоские блоки как в Paint. Вместо — «premium modern design, clean layout, high visual contrast, bold editorial advertising» + доверие модели. **И НЕ сплить на зоны:** `фото top 55–60%` + `solid color band spans full width, lower 40%` + `flat colors / sharp edges` = чёрный слэб с жёстким горизонтальным швом, будто пришит кодом (проверено 01.07 на календарном креативе; кода НЕ было — gpt-image-2 отрисовал ровно заказанный сплит). Текст должен жить ВНУТРИ сцены — на её же затемнённой ГРАДИЕНТНОЙ части (тени кадра), без отдельной плашки и без шва. Ноль процентов-зон, ноль «solid band», ноль «flat/sharp». «trust the model» НЕ отменяет конкретную геометрию — если написал зоны, модель сделает слэб.
