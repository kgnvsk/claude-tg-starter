...
**Вайб — можно, геометрию — нельзя.** НЕ пиши зоны/проценты («top 55%», «lower 40%»), «solid color band spans full width», «flat solid colors / sharp edges», «3 bars», точные раскладки-шаблоны — это арт-режиссура, она и ломает результат в «пейнт» / чёрный слэб с швом. Не лезь в раскладку — codex сам. **Бриф — КОРОТКИЙ (несколько фраз, не эссе на 40 строк: детальная проза тоже зажимает codex) и СИЛЬНЫЙ** (для рекламы — дерзкий/контрастный, НЕ muted/calm/pastel = «слабовато»); НЕ проси отдельную инфо-карточку/баннер с CTA (codex лепит её наклейкой поверх) — оффер интегрируй в кадр.
