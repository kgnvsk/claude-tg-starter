...
const TG_ESCAPE_BIN = process.env.TG_ESCAPE_BIN ?? '/home/claude/bin/tg-escape'
      ...
      case 'reply': {
        ...
        for (let i = 0; i < chunks.length; i++) {
          const shouldReplyTo =
            ...
            replyMode !== 'off' &&
            (replyMode === 'all' || i === 0)
          const replyParams = shouldReplyTo ? { reply_parameters: { message_id: reply_to } } : {}
...
setInterval(() => {
  ...
  if (orphaned) shutdown('orphan-watchdog')
}, 5000).unref()
...
async function lateBoundPhotos(chat_id: string): Promise<string[]> {
  const rows = recentObservedPhotosQuery.all(
    chat_id, Date.now() - LATE_BIND_PHOTO_WINDOW_MS,
  ) as { file_id: string }[]
  const paths: string[] = []
...
async function handleInbound(
  ...
  const latePaths = imagePath == null && downloadImage == null && ctx.chat?.type !== 'private'
    ? await lateBoundPhotos(chat_id)
    : []

  // image_path goes in meta only — an in-content "[image attached — read: PATH]"
  // annotation is forgeable by any allowlisted sender typing that string.
  // Awaited (not fire-and-forget): guarantees the inbound is handed to Claude
  // before the handler returns. On failure we log loudly; bot.catch keeps polling.
  try {
...
let dropPendingOnce = true  // drop backlog only on the FIRST successful poll, never on transient-error retries
void (async () => {
  ...
  if (process.env.TG_TRANSPORT === 'daemon') {
    ...
    while (!shuttingDown) {
      ...
      for (const n of names) {
        ...
        try {
          const update = JSON.parse(readFileSync(f, 'utf8'))
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      await bot.start({
        drop_pending_updates: dropPendingOnce,  // boot-only: skip backlog on cold start; transient-retry must NOT drop (would lose msgs during outage)
        onStart: info => {
