...
# node 22+ (vercel/npm + playwright-MCP браузер). Ставимо явно з nodesource:
...
> 💡 **Два пути к конфигу ядра — выбери по тому, КТО ставит:**
> - **Человек у клавиатуры на сервере →** `cd /opt/claude-tg-starter && bash onboard.sh`. Спросит ВСЁ интерактивно (имя агента, владелец, токен бота, TZ, голос/календарь), запишет `agent.env` (chmod 600), сам запустит install-core. Потом сразу к **3c**.
> - **Ты — агент, ставишь через SSH (headless) →** НЕ запускай `onboard.sh`: он интерактивен (`read`-промпты, ты на них не ответишь и подвиснешь). Собери данные у владельца в чате, потом запусти install-core ОДНОЙ командой с env-переменными — это **шаг 3b ниже**.
...
> С 2026-07-01 интерактивный Google (календарь по запросу, Google Meet `--with-meet`, Gmail, Docs, Sheets) идёт через `gog` + скилл `google-workspace` — ставится install-core автоматически, владелец подключает аккаунт через чат. gcalcli ниже нужен ТОЛЬКО если владелец попросит утренний дайджест — кит этот крон больше НЕ ставит, агент добавляет его сам по просьбе (`30 7 * * * ~/bin/cash-morning-calendar`).
