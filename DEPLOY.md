...
**Дизайн-скиллы (`impeccable`, `vercel-deploy`, `codex-imagegen`):** уже разложены в `~/.claude/skills/` установщиком (Фаза 3). `impeccable` использует node-скрипты — убедись, что node стоит (Фаза 1). Verify: `ls ~claude/.claude/skills` → три папки; бот по просьбе «задизай лендинг» подхватывает `impeccable`.
...
⚠️ **Коннекторы claude.ai (Gmail/Calendar/Drive) в headless-боте НЕ работают.** Их тулзы не успевают подгрузиться до заморозки снапшота доступных инструментов в `--channels` / `claude -p` (баги #36833 / #43298). **Не обещай владельцу, что они «появятся автоматически»** — для интерактивного десктопа владельца они есть, для серверного бота нет. Календарь — через **локальный gcal** (cached-OAuth владельца, headless-proof; rework на это: план v2 D2/D4). Не закладывайся на claude.ai-коннекторы в кронах/боте.
