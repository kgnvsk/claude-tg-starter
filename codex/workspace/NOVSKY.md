...
Store new passwords and keys only through the integration's protected helper in its designated .env or credential store. Do not duplicate them in code, vault notes, ordinary settings or logs. Memory records only the service, purpose and reconnection procedure, without credential values. Novsky keeps its own credentials in its encrypted secret store; do not copy those into agent files. Keep OAuth in the CLI's native credential store.

For an ordinary backup request, use the backup-recovery skill and ~/bin/agent-full-backup. The private archive preserves agent data and available conversation history while excluding credential stores. Reconnect services from reconnect-inventory.json after restoration. Check the actual coverage and legacy-data redaction report; never describe a partial memory projection as a full backup.
