...
ask_opt OPENAI_API_KEY "OpenAI API key" "голосовые транскрипции. Пусто → выкл."
...
echo "── GitHub (бэкап памяти + веб-vault) ──"
...
case "${__gh:-}" in
  y|Y|yes|да|Да)
    ...
    echo "     тогда Vercel-деплой не упрётся в COMMIT_AUTHOR_REQUIRED."
    if command -v gh >/dev/null 2>&1; then
      echo "  2) Логин: сейчас откроется gh auth login (device flow) — он покажет код,"
      echo "     ты вставишь его в браузере и подтвердишь. Терминал ничего не хранит."
    ...
    ;;
  *) echo "  ↳ пропущено. Без GitHub бэкап памяти и веб-vault не заработают — настроишь позже." ;;
esac
echo
